...
Markdown.dialects.Maruku.block.block_code = function block_code( block, next) 
{
	var m = block.match(/(~~~~)("[^"]+")?([\s\S]+)/);
	if (m)
	{
		...
		while (!endfound)
		{
			...
			if (m)
			{
				//Hokay, we're done searching.
				res += "\n\n" + m[1];
				endfound = true;
			} else
			{
				var added = blk.valueOf();
				res += "\n\n" + added;
			}
		}
		...
		{
			return [
				["code_block", { class: "brush: " + brush.replace(/"/gi, '')}, res]
			];
		} else 
		{
			return [["code_block", res]];
		}
	}
	return undefined;
}
